...
import com.xincao.common.nio.core.AionConnection.State;
import com.xincao.common.nio.packet.client.AionClientPacket;
import com.xincao.common.nio.packet.client.CLIENT_TEST;
import com.xincao.common.nio.packet.server.AionServerPacket;
import com.xincao.common.nio.packet.server.SERVER_TEST;
...
public class AionPacketHandler {
	...
	private static final Logger log = LoggerFactory
			.getLogger(AionPacketHandler.class);
	 ...
	 * @return
	 */
	public static AionServerPacket getServerPacketByASK(AionServerKind ask,
		...
		if (ask.getState() == state) {
			try {
				...
				Constructor<AionServerPacket> constructor = clazz
	...
	public static AionClientPacket handle(ByteBuffer data, AionConnection client) {
		...
		if (ack != null) {
			try {
				...
				Constructor<AionClientPacket> constructor = clazz
						.getConstructor(ByteBuffer.class, AionConnection.class,
								Integer.class);
	...
	private static void unknownPacket(State state, int id) {
		log.warn(String.format(
				"Unknown packet recived from Aion client: 0x%02X state=%s", id,
				state.toString()));
	}
}